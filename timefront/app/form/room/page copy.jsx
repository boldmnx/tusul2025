...
export default function Room() {
  ...
  const startEdit = (r) => {
    ...
    setRoomNumbers(
      Array.isArray(r.room_number) ? r.room_number.map((n) => n.id).join(", ") : ""
    );
  };
    ...
    <div className="max-w-5xl mx-auto p-6 space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex items-center justify-between border-b border-slate-100 pb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-black text-slate-800 tracking-tight uppercase">Өрөөний зохион байгуулалт</h2>
            <p className="text-slate-500 text-sm font-medium">Танхимын төрөл болон дугааруудыг удирдах</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden">
        <form onSubmit={saveRoom} className="p-8 flex flex-col md:flex-row items-end gap-6">
          <div className="w-full md:w-1/3 space-y-2">
            <label className="text-xs font-bold text-slate-400 uppercase ml-1">Өрөөний төрөл</label>
            <select
          ...
          <div className="w-full md:w-2/3 space-y-2">
            <label className="text-xs font-bold text-slate-400 uppercase ml-1">Өрөөний дугаарууд (Таслалаар тусгаарлах)</label>
            <input
              type="text"
          ...
          <div className="flex gap-2 w-full md:w-auto">
            {editId && (
              <button
                type="button"
                onClick={() => { setEditId(null); setRoomType(""); setRoomNumbers(""); }}
                className="px-6 py-4 text-slate-500 font-bold hover:bg-slate-100 rounded-2xl transition"
              >
                Болих
              </button>
            )}
            <button className={`whitespace-nowrap px-10 py-4 rounded-2xl font-bold text-white shadow-lg transition-all active:scale-95 ${editId ? "bg-amber-500 shadow-amber-100" : "bg-rose-500 shadow-rose-100 hover:bg-rose-600"}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {rooms.map((r) => (
          <div key={r.id} className="bg-white rounded-[2rem] border border-slate-200 p-6 hover:shadow-xl hover:shadow-slate-200/50 transition-all group relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => startEdit(r)} className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-amber-500 transition-colors">✏️</button>
                  <button onClick={() => deleteRoom(r.id)} className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-red-500 transition-colors">🗑️</button>
                </div>
              </div>

              <div className="flex flex-wrap gap-2 pt-2">
                {Array.isArray(r.room_number) && r.room_number.map((n) => (
                  <span key={n.id} className="bg-slate-100 text-slate-700 px-3 py-1.5 rounded-lg text-sm font-bold border border-slate-200 shadow-sm">
                    {n.id}
                  </span>
                ))}
              </div>
        ...
        {rooms.length === 0 && (
          <div className="col-span-full py-20 text-center border-2 border-dashed border-slate-200 rounded-[2rem]">
            <p className="text-slate-400 font-medium font-lg italic">Бүртгэлтэй танхим одоогоор алга.</p>
          </div>
        )}
      </div>
    </div>
