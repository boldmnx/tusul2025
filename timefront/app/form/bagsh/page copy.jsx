...
export default function Bagsh() {
  ...
  const [checkingAuth, setCheckingAuth] = useState(true); // Session шалгах төлөв

  
  // 1️⃣ Session шалгах
  useEffect(() => {
    fetch("http://127.0.0.1:8000/api/current_user/", { credentials: "include" })
      .then((res) => {
      ...
      .catch(() => {
        ...
        router.push("/auth/signin"); // Нэвтрээгүй бол /signin рүү
      });
  }, [router]);

  // 2️⃣ Багш нарын мэдээллийг авах
  const getTeachers = () => {
  ...
  useEffect(() => {
    if (!checkingAuth) getTeachers();
  }, [checkingAuth]);

  // 3️⃣ Хэрэглэгч шалгалт хийж байвал loading
  if (checkingAuth) {
    return (
  ...
  // 4️⃣ Үлдсэн компонентын кодыг өөрчлөлтгүй ашиглаж болно
  ...
  const deleteTeacher = (id) => {
    if (confirm("Та энэ багшийг устгахдаа итгэлтэй байна уу?")) {
      fetch("http://127.0.0.1:8000/service/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "deleteTeacher", id: id }),
        credentials: "include",
      }).then(() => {
        setSelectedTeacher(null);
        getTeachers();
  ...
  const startEdit = () => {
    ...
    setPreview(selectedTeacher.photo_url || null);
    setIsEditing(true);
  };
